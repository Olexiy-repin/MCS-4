...
const createCarCard = carInfo => {
  ...
  */
  ...
  return `
  <li class="car-list-item">
    <img class="car-list-img" src="${carInfo.img}" alt="${carInfo.car} ${carInfo.model}" />
    <div class="car-list-text-content">
...
const template = allCars.map(car => createCarCard(car)).join('');

galleryListEl.innerHTML = template;

const onSearchFormElSubmit = event => {
  event.preventDefault();

  const formData = {
    [searchFormEl.elements.query.name]: searchFormEl.elements.query.value,
    [searchFormEl.elements.options.name]: searchFormEl.elements.options.value,
  };

  console.log(formData);

  const filteredCars = allCars.filter(car => {
    return car[formData.options]
      .toLowerCase()
      .includes(formData.query.toLowerCase());
  });

  const template = filteredCars.map(car => createCarCard(car)).join('');
